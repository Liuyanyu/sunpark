        ...
        <div class="main">
            <div class="container">
                <div class="nav-btns text-center pad-top-40 pad-bottom-40" id="btns">
                    ...
                    <router-link class="button active" :to="{path: '/booking/other#btns'}">其它票务</router-link>
                </div>
                <div>
                    <a :href="item.otherLink || 'javascript:;'" :class="{'project':true, 'cursor-default': !item.otherLink}" v-for="(item, index) in list">
                        <div class="row">
                            <div class="col-4 project-title pad-left-10">
                                <div class="text-table">
                                    <div class="text-table-cell">
                                        {{item.otherName}}
                                        <small class="none block-l pad-top-20" v-html="item.otherIntro"></small>
                                    </div>
                                </div>
                            </div>
        ...
        queryList($state){
            api.queryBookingOther(this.pageNum, 1).then(res => {
                ...
                const newList = object.webBookingOtherList.map(item => {
                    ...
                    item.otherIntro = formatText(item.otherIntro)
                    return item
                })
